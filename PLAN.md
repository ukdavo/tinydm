...
| 9.3 | Azure Blob backend + tests | ✅ | `internal/storage/azure.go` — `github.com/Azure/azure-sdk-for-go/sdk/storage/azblob`; Azurite-compatible via custom endpoint; dedup via `GetProperties`. Tests in `azure_test.go` skip unless `TINYDM_AZURE_TEST_ENDPOINT` is set. Config: `TINYDM_AZURE_ACCOUNT`, `TINYDM_AZURE_KEY`, `TINYDM_AZURE_CONTAINER`, `TINYDM_AZURE_ENDPOINT` |
| 9.4 | GCS backend + tests | ✅ | `internal/storage/gcs.go` — `cloud.google.com/go/storage`; `NewGCSWithClient` for test injection; dedup via `Attrs()`. Tests in `gcs_test.go` use `fake-gcs-server/fakestorage` in-process (no Docker), 13 tests mirroring S3 suite. Config: `TINYDM_GCS_BUCKET`, `TINYDM_GCS_PROJECT`, `TINYDM_GCS_CREDENTIALS_FILE` |
| 9.5 | `cluster.Locker` interface + implementations | ✅ | `internal/cluster/locker.go`: `Locker` interface, `NoOpLocker` (SQLite/single-node), `PGLocker` (session-level `pg_advisory_lock` / `pg_advisory_unlock`; FNV-1a key→int64); selected by DB driver in `main.go` |
| 9.6 | Wire document locking into write paths | ✅ | `DocumentHandler.Upload` (bucket lock), `Update`, `RestoreVersion` (document lock); `TagHandler.Replace`, `PropertyHandler.Replace` (document lock); `RegisterRoutes` + all handler constructors accept `cluster.Locker`; tests use `NoOpLocker` |
| 9.7 | Leader election — `cluster.LeaderElector` | ✅ | `internal/cluster/leader.go`: `LeaderElector` interface; `NoOpLeaderElector` (always leader); `PGLeaderElector` (dedicated conn + `pg_try_advisory_lock`; 5 s heartbeat goroutine; graceful `Stop()`). DB migration `003_cluster_nodes.sql` (both SQLite + PG). `TINYDM_NODE_ID` env var (default: hostname) |
| 9.8 | Enhanced `/health` endpoint | ✅ | `Ping(ctx) error` added to `storage.Store` interface; implemented on `Local` (stat), `S3Store` (HeadBucket), `AzureStore` (container GetProperties), `GCSStore` (Attrs). Health handler probes DB + storage; response: `{"status","version","commit","node_id","db","storage"}` |
| 9.9 | Cluster docker-compose + nginx config | ✅ | `docker-compose.cluster.yml`: 3 tinydm nodes + nginx + postgres + MinIO; `nginx.cluster.conf`: `least_conn` upstream, keepalive, `client_max_body_size 512m` |
| 9.10 | Update DEPLOYMENT.md with cluster section | ✅ | Added: prerequisites table, `TINYDM_NODE_ID`, quick start, nginx upstream snippet, leader election notes, enhanced health response, rolling upgrade procedure |
| 9.11 | Cluster integration test | ✅ | `test_cluster.sh`: starts 3-node compose cluster; tests health shape, cross-node upload/download (node-1 → node-2), 5 concurrent uploads, leader election (both nodes healthy); `--keep` flag for debugging |
